...
def generate_annotation(
    img_folder_rgb, img_folder_dhs, label_folder, annotation_filename
):
    """
    ...
    """
    img_files_rgb = sorted(glob.glob(img_folder_rgb + "*.png"))
    img_files_dhs = sorted(glob.glob(img_folder_dhs + "*.png"))
    img_files = img_files_rgb + img_files_dhs
    random.seed(42)
...
def convert_data_to_coco_style(dataset_folder):
    ...
    """
    # For train
    img_folder_rgb = dataset_folder + "train/rgb/"
    img_folder_dhs = dataset_folder + "train/dhs/"
    label_folder = dataset_folder + "train/gts/raw_gts/"
    generate_annotation(
        img_folder_rgb, img_folder_dhs, label_folder, "det_train_rgb_and_dhs.json"
    )
